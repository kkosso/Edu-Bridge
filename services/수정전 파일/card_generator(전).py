...
기존 prompt_card_generator.py는 "5개국 중 차별화된 3개국 선택"으로 설계됐으나,
3개국 정책으로 변경됨에 따라 본 파일에서 시스템 프롬프트를 일부 오버라이드해서
"주어진 3개국 모두 카드로 만들기"로 동작하게 함.
...
사용 예:
    ...
    result = generate_cards(
        ...
        age=4,
        duration=40,
        top_countries=retriever_result["top_countries"],   # 3개국
        countries_metadata=metadata_dict,                  # {코드: 국가데이터}
    )
...
from prompts.prompt_card_generator import (
    build_prompt as _build_prompt_5country,
    USER_MESSAGE_TEMPLATE as _USER_TEMPLATE_5,
...
def _build_prompt_3country(
    ...
    """3개국 카드 생성용 프롬프트 빌드 (기존 5국 빌더의 country block 재사용)."""
    country_blocks = []
...
def generate_cards(
    ...
    """
    retriever 결과의 top_countries (3개국)를 카드 3장으로 변환.

    Args:
        user_query: 사용자 활동 의도 (search_query)
        age: 만 나이
        duration: 활동 시간(분)
        top_countries: retriever.search() 결과의 top_countries (길이 3 권장)
        countries_metadata: {country_code: country_dict} 형태로 사전 변환된 메타데이터
...
# ============================================================
# 헬퍼: countries_metadata 사전 변환
# ============================================================
def load_countries_metadata(all_countries_path: Path) -> dict:
    """all_countries.json을 읽어 {국가코드: 국가데이터} 형태로 변환."""
...
if __name__ == "__main__":
    # retriever를 실제로 돌려서 top-3을 받아온 뒤 카드 생성 테스트
    ...
    print("=" * 70)
    print("card_generator.py 단독 테스트")
    print("=" * 70)
    ...
    for query, age, duration in test_queries:
        ...
        try:
            ...
            for card in result["cards"]:
                ...
                print(f"           tag: {card['philosophy_tag']}")
